...
@implementation NSObject (LNModel)

+ (instancetype)ln_modelWithDictionary:(NSDictionary *)dictionary {
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        if ([propertyNameStr isEqualToString:@"courses"]) {
        }
        // 对特殊属性进行处理
        ...
        if ([value isKindOfClass:[NSDictionary class]] && ![propertyType hasPrefix:@"NS"]) {
            ...
            if (modelClass != nil) {
                // 将被嵌套字典数据也转化成Model
                value = [modelClass ln_modelWithDictionary:value];
            }
        }
        ...
        if ([value isKindOfClass:[NSArray class]] && perpertyTypeDic) {
            ...
            for (NSDictionary *itemDic  in value) {
                id model = [itemModelClass ln_modelWithDictionary:itemDic];
